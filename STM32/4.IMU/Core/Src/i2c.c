...
void I2C_Receive(I2C_TypeDef* I2C, uint8_t address, uint8_t* outputdata, uint16_t size){
	...
	LL_I2C_DisableIT_RX(I2C);							//Disable Interrupt
	...
	if(size==1){										// when data size == 1
		LL_I2C_AcknowledgeNextData(I2C, LL_I2C_NACK);	// prepare to generate Non ACKnowledge after data receive
	}else{												// when data size > 1
		LL_I2C_AcknowledgeNextData(I2C, LL_I2C_ACK);	// ACKnowledge
	}
	...
	for(uint16_t i=0; i<size; i++){
		...
		while(!LL_I2C_IsActiveFlag_RXNE(I2C));			// wait to complete receiving data
		outputdata[i] = LL_I2C_ReceiveData8(I2C);		// save data in array
	}
	LL_I2C_GenerateStopCondition(I2C);					// Stop bit
}
